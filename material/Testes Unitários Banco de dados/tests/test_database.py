...
class TestDataBase(TestCase):
    ...
    def test_select_data_works(self, mock_conn_and_cursor_exist, mock_is_database_selected):
        ...
        with mock.patch("app.database.DataBase.cursor", create=True) as mock_cursor:
            mock_cursor.execute.side_effect = [0, 1]
            self.assertEqual(DataBase().select_data("", 0), list())


        self.assertFalse(DataBase().select_data("", 0), "Nenhuma registro encontrado !")

    @mock.patch("app.database.DataBase.is_database_selected")
    @mock.patch("app.database.DataBase.conn_and_cursor_exist")
    def test_convert_dict_sql_works(self, mock_conn_and_cursor_exist, mock_is_database_selected):
        with mock.patch("app.database.DataBase.cursor", create=True) as mock_cursor:
            mock_cursor.execute.side_effect = [0, 1]
            self.assertEqual(DataBase().convert_dict_to_sql("", dict(id=0, nome="bla")), False)
            self.assertEqual(DataBase().convert_dict_to_sql("", dict(id=0, nome="bla")), True)

        self.assertFalse(DataBase().convert_dict_to_sql("", dict(id=0)))
